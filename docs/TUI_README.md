...
```bash
# 1) Start both MCP servers on custom ports
COMFY_MCP_HTTP_BIND_PORT=8101 \
PHOTARIUM_HTTP_PORT=8790 \
PHOTARIUM_ROOT=/Users/julian/Code/cloud-flare-image-handler \
./run_mcp_processes.sh restart

# 2) Update mcp_chat_config.json http_url values to :8101 and :8790

# 3) Start TUI with same port env vars
COMFY_MCP_HTTP_BIND_PORT=8101 PHOTARIUM_HTTP_PORT=8790 ./run_mcp_chat.sh
```
